...
import {
  ...
  IonText, IonSpinner, IonButton, IonItem, IonLabel,
  IonInput, IonTextarea, IonDatetime, IonToggle,
...
export class FormFillPage implements OnInit {
  ...
  loadSchemaOnly() {
    ...
    this.api.getLatestFormVersion(this.tenantId, this.idForm).subscribe({
      ...
      error: (err) => {
        this.errorMsg = err?.error?.message || err?.message || 'Erro ao carregar versão do formulário';
        this.loading = false;
      }
    });
  }
...
private normalizeSchema(schemaAny: any): any | null {
  ...
  private flattenElements(schemaAny: any): any[] {
    ...
    const root = schema?.schema_json ? this.normalizeSchema(schema.schema_json) : schema;
  ...
  async save() {
    ...
    if (!this.formVersion?.id_form_version) {
      this.errorMsg = 'Formulário ainda não carregou corretamente (sem versão).';
      return;
    }
    ...
    try {
      ...
      if (!this.submissionId) {
        ...
        if (!this.submissionId) {
          throw new Error('Falha ao criar submission (id_form_submission não retornou).');
        }
      }

      // 2) Salva payload
      await this.api.saveSubmissionPayload(this.submissionId, this.tenantId, this.payload).toPromise();
   ...
   */
  canSave(): boolean {
    return !this.loading && this.elements.length > 0 && !this.saving && !this.errorMsg;
  }
}
